...
SELECT ModuleName, Subject FROM module;  -- This is the easiest way to do this. No join is needed.

SELECT ModuleName, Subject, course.CourseID FROM module
LEFT JOIN course
ON module.CourseID = course.CourseID
ORDER BY CourseID;  -- The same outcome but using a join. A minor difference in results due to two modules allocted to Chemistry not having a CourseID
